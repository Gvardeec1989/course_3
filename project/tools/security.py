...
def compare_passwords_hash(password_hash, other_password):
    """
    метод возвращает сравнение бинарных последовательностей чисел
...
def generate_tokens(email, password, password_hash=None, is_refresh=False):
    """
    метод который генерирует access_token и refresh_token,
    получая email и password пользователя с проверкой is_refresh
    (сщздание новых тонеков, ане перегенерация refresh_token)
    """
    if email is None:
        return None
